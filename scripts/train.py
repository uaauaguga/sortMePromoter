...
def main():
    ...
    parser.add_argument('--kernel-size', '-k', type=int, default=3, help="convolution kernel size in the res-block")
    ...
    train_set = PromoterSet(args.train_positive, args.train_negative, length=args.length)
    train_sampler = RandomSampler(train_set)
    train_loader = DataLoader(train_set, sampler=train_sampler, batch_size=256, collate_fn=collate_fn)

    val_set = PromoterSet(args.val_positive, args.val_negative, length=args.length)
    val_sampler = RandomSampler(val_set)
    val_loader = DataLoader(val_set, sampler=val_sampler, batch_size=256, collate_fn=collate_fn)
    ...
    for e in range(50):
        ...
        for sequences, labels, group_ids in train_loader:
            ...
            if i%256 == 0:
                model = model.eval()
                logger.info(f"train: {np.mean(train_losses)} {accuracy.item()}")
                train_losses = []
                records = []
                correct, N = 0, 0
                losses = []
                ...
                logger.info(f"val: {np.mean(losses)} {correct/N} {AUROC}")
                model = model.train()
            i += 1
